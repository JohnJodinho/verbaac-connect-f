...
import { PageWrapper, AnimatedButton, PageLoader } from '@/components/animated';
...
const AddressDisplay = ({ address }: { address: string }) => {
    ...
    if (isAuthenticated) return <span>{address}</span>;
    
    // Simple mock masking logic
    const parts = address.split(',');
    const generalArea = parts.length > 1 ? parts[parts.length - 2].trim() : address;
    
    return (
        <span className="inline-flex items-center">
            {generalArea}, Jos
            <span className="ml-2 text-xs bg-muted px-1.5 py-0.5 rounded text-muted-foreground italic">
    ...
    <PageWrapper>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between w-full">
          <div>
            ...
            <p className="text-sm text-muted-foreground"><AddressDisplay address={property.location.address} /></p>
          </div>
            <div className="flex items-center gap-4">
            <span className="text-2xl font-bold text-primary">{formattedRent}</span>
             <GuestLock fallbackText="Log in" blur={false} className="min-w-[100px]">
                <AnimatedButton>
          ...
          <main className="lg:col-span-2 space-y-8">
            ...
            <section id="header" className="pb-6 border-b border-border">
              ...
              <p className="text-lg text-muted-foreground mb-4">
                {property.propertyType} • <AddressDisplay address={property.location.address} />
              </p>
          ...
          <aside className="lg:col-span-1">
            <div id="action-panel-desktop" className="hidden lg:block sticky top-28 bg-card border border-border rounded-lg shadow-xl p-6">
              ...
              <div className="flex flex-col gap-3 mt-6 relative">
                 <GuestLock fallbackText="Sign in to contact landlord" blur={true}>
      ...
      >
        <div className="flex justify-between items-center">
          ...
          <GuestLock fallbackText="Log in to chat" blur={false} className="min-w-[120px]">
